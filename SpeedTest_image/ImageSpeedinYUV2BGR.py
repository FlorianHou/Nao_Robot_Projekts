...
with open("SpeedTest_image/YUV2BGR.csv", "wb") as f:
    writer = csv.writer(f)
...
for res in ResList:
    ...
    # Subscribe Kamera
    nameId = video_cam.subscribeCamera("Kamera_Get78", CamId, res, ColorSpace, fps)
    ...
    for round in range(10):
        ...
        for f in range(200):
            ...
            img_BGR = cv.cvtColor(img_buffer.reshape(contain[1],contain[0],-1), cv.COLOR_YUV2BGR_YUYV)
            # cv.imshow("", img_BGR)
            # cv.waitKey(1)
